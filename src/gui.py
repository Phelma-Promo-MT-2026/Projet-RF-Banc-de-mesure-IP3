...
class Log:
    def __init__(self, text_widget):
        # Simple logger that writes text messages into a Tkinter Text widget
        self.text = text_widget

    def write(self, msg: str):
        # Append a message to the log widget and keep the view scrolled to the end
        if not msg.endswith("\n"):
            msg += "\n"
...
class MainWindow(tk.Tk):
    def __init__(self):
        # Main application window: sets up GUI, backends and plotting areas
        super().__init__()
        style = ttk.Style(self)
        # style.theme_use('clam')  # theme that handles borders nicely

        # Configure a custom style for labeled frames used as sections
        style.configure(
            "Section.TLabelframe",
            borderwidth=3,
            relief="groove"
        ...
        # Error manager and RF / DSP backend objects
        ...
        # Default RF / measurement parameters used to pre-fill the GUI fields
        self.f_rf = 2.4e9
        self.delta_f = 1e6
        self.pe = -10.0
        self.g_rx = 0.0
        ...
        # Left column: parameter entry fields and control buttons
        ...
        # Buttons to trigger TX and RX operations
        ttk.Button(
            frame_measure,
            text="Send TX",
        ...
        ttk.Button(
            frame_measure,
            text="Receive RX",
            command=self.receive_rx
        ).pack(pady=10, fill="x")

        # === Section 2 : IP3 COMPUTE SECTION ===
        # Section to manually enter measured tones and compute IIP3
        ...
        # Right column: plots for TX/RX spectra and log area
        ...
        # Separate frames for TX (theoretical) and RX (measured) plots
        ...
        # Now that Log exists, connect ErrorManager to GUI logging
    ...
    def _read_params(self):
        ...
        if (
            not self.entry_f_rf.get()
            or not self.entry_pe.get()
            or not self.entry_p_ton_meas.get()
            or not self.entry_p_imd3_meas.get()
        ):
            self.err_mgr.error("Please fill in all required fields.")
            return None, None, None, None

        # Parse RF and measurement parameters from UI entries
        f_rf = int(float(self.entry_f_rf.get()))
        delta_f = int(1e6)      # fixed tone spacing (for now)
        pe = int(float(self.entry_pe.get()))
        g_rx = int(0)           # fixed RX gain (for now)
        n_sample = int(100000)  # fixed number of samples
        fs = int(4e6)           # fixed sample rate at 4 MHz for now

        p_ton_meas = float(self.entry_p_ton_meas.get())
        p_imd3_meas = float(self.entry_p_imd3_meas.get())

        # Build TX and RX parameter objects used by the bench backend
        tx = TxParams(
            f_rf=f_rf,
            delta_f=delta_f,
            fs=fs,
    ...
    def send_tx(self):
        # Trigger TX signal generation, configuration and plotting of theoretical TX spectrum
        ...
        if freq_abs is None:
            # Early exit if TX sending failed (e.g., calibration issue)
            return

        # Update TX plot with new spectrum
        self.ax_tx.clear()
    ...
    def receive_rx(self):
        # Trigger RX capture and plotting of measured RX spectrum
        if self.bench is None or not self.rx_iface or not self.rx_iface.is_connected():
        ...
        if rx_samples is None:
            # RX failed or not configured; nothing to plot
            return

        # Compute FFT of received samples and update RX plot
        freq_abs, P_bin, P_dBm, A_sample = self.bench.signal_utils.compute_fft(rx_samples, rx.fs, rx.f_rf)
        
        p_tone_pos, p_tone_neg, p_im3_pos, p_im3_neg = self.bench.signal_utils.search_peak_in_band(
            freq_abs,
            A_sample,
            tx.f_rf,
            tx.delta_f/2,
        ...
        self.err_mgr.info(f"Measured tone levels: {p_tone_pos:.2f} , {p_tone_neg:.2f}")
        self.err_mgr.info(f"Measured IMD3 levels: {p_im3_pos:.2f} , {p_im3_neg:.2f}")

        # Update RX plot with new spectrum
        self.ax_rx.clear()
        self.ax_rx.plot(freq_abs, A_sample)
        self.ax_rx.set_xlabel("Hz")
        self.ax_rx.set_ylabel("ADC codes")
        self.ax_rx.grid(True)
        self.canvas_rx.draw()

    def compute_iip3(self):
        # Compute IIP3 from manually entered tone and IMD3 levels
        tx, rx, p_ton_meas, p_imd3_meas = self._read_params()

        # Basic IP3 formula based on delta between fundamental and IM3
        delta_db = p_ton_meas - p_imd3_meas
        ip3_dbm = p_ton_meas + (delta_db / 2.0)

        # Display computed IP3 value in the result entry
        self.entry_result_ip3.delete(0, tk.END)
        self.entry_result_ip3.insert(0, f"{ip3_dbm:.2f}")
        pass

    def clear_log_messages(self):
        # Clear all log messages from the log Text widget
...
class ParameterWindow:
    def __init__(self, parent):
        # Placeholder for a separate parameter configuration window
        self.parent: MainWindow  # reference to the main application window
        # Widgets to be added: entry_delta_f, entry_n_sample, entry_f_sample, entry_f_rf, entry_pe, entry_g_rx
        pass

    def _read_fields():
        # Placeholder: read parameter fields from this window and return a typed dict
        pass

    def apply_and_close():
        # Placeholder: apply the edited parameters to the parent and then close the window
        pass

    def save_defaults():
        # Placeholder: save current parameters as defaults into settings.json
        pass

    def on_close():
        # Placeholder: handle window close event (apply if needed, then destroy)
        pass
